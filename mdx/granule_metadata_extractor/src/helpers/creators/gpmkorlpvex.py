...
#                                                 recreate gpmkorlpvexRefData.json
...
def get_tar_metadata(filename, file_path):
    ...
    elem_path = os.path.join('/tmp', 'gpmkorlpvex')
...
def write_to_dictionary(file_path, temporal, wnes_geometry):
    ...
    metadata[key]['format'] = format_switcher.get(re.search('^lpvex_RADAR_KORPO_(.*)_\d{8}.tar.gz$', file_path.split('/')[-1])[1])
...
if __name__ == "__main__":
    ...
    file_dir = '/ftp/public/pub/fieldCampaigns/gpmValidation/lpvex/C-band_radar/KOR/data/'
    print(file_dir)
    ...
    #print(metadata)
    with open('../gpmkorlpvexRefData.json', 'w') as fp:
