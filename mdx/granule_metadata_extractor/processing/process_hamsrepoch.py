...
class ExtractHamsrepochMetadata(ExtractNetCDFMetadata):
    """
    A class to extract hamsrepoch 
    """
    ...
    def get_variables_min_max(self, nc):
        ...
        :return:
        """
        time_arr = nc.variables['time'][:] #seconds since 2000-01-01 00:00:00.0
        time_ref = datetime.strptime(nc.variables['time'].comment,'seconds since %Y-%m-%d %H:%M:%S.%f')
        ...
        minTime = time_ref+timedelta(seconds=time_arr.min())
        maxTime = time_ref+timedelta(seconds=time_arr.max())
        maxlat = lat_arr.max()
        minlat = lat_arr.min()
        maxlon = lon_arr.max()
        minlon = lon_arr.min()
