...
class ExtractGpmlipiphxASCIIMetadata(ExtractASCIIMetadata):
    """
    A class to extract ASCII metadata 
    """
    metadata = {}
    max_lat = -90.0
    min_lat = 90.0
    ...
    def get_variables_min_max(self, variable_key):
        ...
        :param c_data: ASCII data
        :param variable_key: The ASCII (filename) key we need to target
        :return: list longitude coordinates
        """
        fname = variable_key.split('/')[-1].split('.')[0]
        key = fname.split('_')[2]
        #print(variable_key, ' ', fname,' ',key)
        try:
...
if __name__ == '__main__':
    ...
    file_path = "/home/lwang13/GHRCCLOUD/granule-metadata-extractor/test/fixtures" \
                "/iphex_lip_14121.txt"
    exnet = ExtractGpmlipiphxASCIIMetadata(file_path)
