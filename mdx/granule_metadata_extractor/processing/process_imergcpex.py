...
class ExtractImergcpexMetadata(ExtractNetCDFMetadata):
    """
    ...
    def get_variables_min_max(self):
        ...
        #Sample file: CPEX_3B_HHR_E_MS_MRG_3IMERG_20170524_S180000_E182959_1080_V04B.nc 
        tkn = self.file_path.split('/')[-1].split('.nc')[0].split('_')
        date_str = tkn[7] #i.e., 20170524
        start_str = ''.join([tkn[7],tkn[8]]) #i.e., 20170524S180000
        end_str = ''.join([tkn[7],tkn[9]]) #i.e., 20170524E182959
        minTime = datetime.strptime(start_str,'%Y%m%dS%H%M%S')
        maxTime = datetime.strptime(end_str,'%Y%m%dE%H%M%S')
