...
class ExtractSbumetimpactsNetCDFMetadata(ExtractNetCDFMetadata):
    """
    ...
    def get_variables_min_max(self, nc, file_path):
        ...
        for i in range(ctime.shape[0]):
            ...
            if ltime_str in loc:
                loc[ltime_str][1] = max(llat, loc[ltime_str][1]) 
                loc[ltime_str][0] = min(llat, loc[ltime_str][0])
                loc[ltime_str][3] = max(llon, loc[ltime_str][3])
                loc[ltime_str][2] = min(llon, loc[ltime_str][2])
            else:
