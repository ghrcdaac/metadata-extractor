...
class ExtractGfscpexMetadata(ExtractNetCDFMetadata):
    """
    A class to extract gfscpex
    """
    ...
    def get_variables_min_max(self):
        ...
        #Sample file: cpex_gfs_2017052400_000.nc
        tkn = self.file_path.split('/')[-1].split('.nc')[0].split('_')
        minTime = datetime.strptime(tkn[2],'%Y%m%d%H')+timedelta(hours=int(tkn[3]))
        maxTime = minTime 
