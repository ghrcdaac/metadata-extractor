...
class ExtractNalmanrtMetadata(ExtractASCIIMetadata):
    """
    A class to extract Nalmanrt granule metadata
    """
    ...
    def get_variables_min_max(self, **kwargs):
        """
        Extracts temporal and spatial metadata from nalmanrt granules
        """
        self.start_time = min(datetime.strptime(re.search(r'^.*(\d{6}_\d{6}).*$',
                                                          self.file_name)[1], '%y%m%d_%H%M%S'),
                              self.start_time)
        self.end_time = max(self.end_time, self.start_time + timedelta(seconds=59))
...
if __name__ == '__main__':
    print('Extracting Nalmanrt Metadata')
