...
class ExtractAuslmarawMetadata(ExtractASCIIMetadata):
    """
    A class to extract auslmaraw granule metadata
    """
    ...
    def __init__(self, file_path):
        ...
        with open(os.path.join(os.path.dirname(__file__), f"../src/helpers/auslmarawRefData.json"),
                  'r') as f:
    ...
    def get_variables_min_max(self, **kwargs):
        """
        Extracts temporal metadata and assign spatial metadata for auslmaraw granules
        """
        ...
        station_identifier = re.search(r'^L([A-Z])_IGNIS_.*_\d{6}_\d{6}.dat(.gz)?$',
                                       self.file_name)[1]
    ...
    def compress_raw_file(self):
        """
        Compresses auslmaraw granules for archiving
        """
...
if __name__ == '__main__':
    print('Extracting auslmaraw Metadata')
