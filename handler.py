...
def extract_browse_metadata(ds_short_name, version, access_url, browse_file, browse_vars={},
    ...
    #     data = metadata.get_metadata(ds_short_name=ds_short_name, format=format_template.get(ds_short_name, format),
    #                                  version=version)
    #     data['OnlineAccessURL'] = access_url
    #     echo10xml = src.GenerateEcho10XML(data)
    #     echo10xml.generate_echo10_xml_file(output_folder=output_folder)
...
def exclude_fetch():
    ...
    :return:
    """
    return ["tcspecmwf", "gpmwrflpvex", "relampagolma", "goesrpltavirisng", "gpmvanlpvex", "gpmikalpvex", "gpmkorlpvex",
            "gpmkerlpvex", "gpmkumlpvex",
...
def task(event, context, temp_path=None):
    ...
    #     generated_files = [output_file_path + ".cmr.xml"] if excluded else [output_file_path,
    #                                                                         output_file_path + ".cmr.xml"]
