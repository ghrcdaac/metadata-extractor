...
class ExtractIsslisgv1Metadata(ExtractNetCDFMetadata):
    """
    A class to extract ISSLIS background
    """
    ...
    def get_nc_metadata(self):
        ...
        min_time, max_time = [ref_time + timedelta(seconds=utc.min()),
        ...
        if 'bg_data_summary_boresight' in datafile.variables.keys():
        ...
        datafile.close()
        return min_time, max_time, minlat, maxlat, minlon, maxlon

    def get_hdf_metadata(self):
        f = HDF(self.file_path)
        vs = f.vstart()
        data = vs.attach('bg_info')
        bg_info = data[:]  # ['TAI93_time', 'alert_summary',......,'bg_value', 'noise_index',
        # 'event_count']
        ...
        try:
            ...
            # corresponding *.nc doesn't have bg_data_summary_[lat|lon|corners|TAI93_time]
            # variables; use on-prem bounding box (hard-coded by Nathan)
            pass
...
if __name__ == '__main__':
    ...
    exnet = ExtractIsslisgv1Metadata(path_to_file)
