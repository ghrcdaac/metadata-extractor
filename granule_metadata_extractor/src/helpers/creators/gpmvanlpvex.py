...
def get_reference_metadata(filename):
    ...
    if '_VANTAA_' in filename:#VAN site provide raw binary files only
        #VAN: 60.33222222 N, 24.90611111 E
        minlat = 60.33222222-0.01
        maxlat = 60.33222222+0.01
        minlon = 24.90611111-0.01
        maxlon = 24.90611111+0.01
    ...
    else: #other 4 radar lpvex sites
...
def write_to_dictionary(file_path, temporal, wnes_geometry):
    ...
    metadata[key]['format'] = format_switcher.get(re.search('^lpvex_RADAR_VANTAA_(.*)_\d{8}.tar.gz$', file_path.split('/')[-1])[1])

if __name__ == "__main__":
    #print(f"Directory Path is {sys.argv[1]}")
    #file_dir = str(sys.argv[1])
    #(1) Extract metadata from UF files first, and assign UF metadata to RAW binary files
    file_dir = '/ftp/public/pub/fieldCampaigns/gpmValidation/lpvex/C-band_radar/VAN/data/'
    print(file_dir)
    ...
    #(2) Assign UF metadata to browse PNG images
    file_dir = '/ftp/public/pub/fieldCampaigns/gpmValidation/lpvex/C-band_radar/VAN/browse/'
    print(file_dir)
    ...
    #print(metadata)
    with open('../gpmvanlpvexRefData.json', 'w') as fp:
