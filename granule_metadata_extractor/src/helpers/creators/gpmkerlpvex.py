...
#                                                 recreate gpmkerlpvexRefData.json
...
def get_tar_metadata(filename, file_path):
    ...
    elem_path = os.path.join('/tmp', 'gpmkerlpvex')
...
def get_reference_metadata(filename):
    ...
    if reference_key in metadata.keys():
        ...
        #KER:NLat=62.63395309448242,SLat=58.14215850830078,WLon=20.565059661865234,ELon=29.6627140045166
        maxlat = 62.63395309448242
        minlat = 58.14215850830078
        minlon = 20.565059661865234
        maxlon = 29.6627140045166
...
def write_to_dictionary(file_path, temporal, wnes_geometry):
    ...
    #lpvex_RADAR_KERAVA_UF_20100921.tar.gz
    metadata[key]['format'] = format_switcher.get(re.search('^lpvex_RADAR_KERAVA_(.*)_\d{8}.tar.gz$', file_path.split('/')[-1])[1])
...
if __name__ == "__main__":
    ...
    file_dir = '/ftp/public/pub/fieldCampaigns/gpmValidation/lpvex/C-band_radar/KER/data/'
    print(file_dir)
    ...
    #print(metadata)
    with open('../gpmkerlpvexRefData.json', 'w') as fp:
