...
# https://ghrc.nsstc.nasa.gov/hydro/details/gpmkpmx2ifld
nlat = 48.99
slat = 40.71
wlon = -99.4
elon = -87.74
...
if __name__ == "__main__":
    file_dir = "/ftp/ops/public/pub/fieldCampaigns/gpmValidation/ifloods/NEXRAD2/KMPX/data/"
    #(1) read in metadata Mary extracted from database
    with open('gpmkmpx2ifld_dbmeta_redone','r') as fp:
         dbmeta = json.load(fp)

    #(2) read the list of renamed files
    rn_files, old_files, new_files = load_renaming_script('KMPX_renaming_script')
    ...
    with open('../gpmkmpx2ifldRefData.json', 'w') as fp:
