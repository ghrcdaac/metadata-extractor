...
#                                                 recreate gpmkumlpvexRefData.json
...
def get_tar_metadata(filename, file_path):
    ...
    elem_path = os.path.join('/tmp', 'gpmkumlpvex')
    for elem in sorted(tar_opened.getmembers(), key=lambda m: m.name):
        ...
        if '_IKA.' in elem_name or '_KOR.' in elem_name:
        #for IKA dataset, i.e., 201010190700_IKA.PPI1_A.raw.uf
        #for KOR dataset, i.e., 201010190700_KOR.PPI1_A.raw.uf
           dt0 =datetime.strptime(elem_name.split('_')[0][:],'%Y%m%d%H%M%S')
        else:
        #for datasets except IKA and KOR
...
def get_reference_metadata(filename):
    ...
    if reference_key in metadata.keys():
        granule_metdata = {'wnes_geometry': metadata[reference_key]['wnes_geometry'],
                       'temporal': metadata[reference_key]['temporal']}
    else:
        #KUM: NLat=64.686806,SLat=55.722157,WLon=15.914511,ELon=34.012142
        maxlat = 64.686806
        minlat = 55.722157
        minlon = 15.914511
        maxlon = 34.012142

        key = filename.split('/')[-1].split('.')[0].split('_')[-1] #i.e., 20100901 (daily)
        mintime = datetime.strptime(key,'%Y%m%d')
        maxtime = mintime + timedelta(seconds=86399)
        start_time = datetime.strftime(mintime, '%Y-%m-%dT%H:%M:%SZ')
        end_time = datetime.strftime(maxtime, '%Y-%m-%dT%H:%M:%SZ')
        granule_metdata = {'wnes_geometry': [str(round(x, 3)) for x in
                                           [minlon, maxlat, maxlon, minlat]],
...
def write_to_dictionary(file_path, temporal, wnes_geometry):
    ...
    #lpvex_RADAR_KUMPULA_UF_20100901.tar.gz
    metadata[key]['format'] = format_switcher.get(re.search('^lpvex_RADAR_KUMPULA_(.*)_\d{8}.tar.gz$', file_path.split('/')[-1])[1])

if __name__ == "__main__":
    #print(f"Directory Path is {sys.argv[1]}")
    #file_dir = str(sys.argv[1])
    # (1)Extract metadata from UF files first, and assgin UF metadata to RAW binary files
    file_dir = '/ftp/public/pub/fieldCampaigns/gpmValidation/lpvex/C-band_radar/KUM/data/'
    print(file_dir)
    ...
    # (2) Assgin UF metadata to PNG browse files
    file_dir = '/ftp/public/pub/fieldCampaigns/gpmValidation/lpvex/C-band_radar/KUM/browse/'
    print(file_dir)
    ...
    #print(metadata)
    with open('../gpmkumlpvexRefData.json', 'w') as fp:
