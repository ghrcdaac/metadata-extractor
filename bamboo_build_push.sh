...
function stop_mdx_task() {
 ./aws ecs stop-task --cluster $1-CumulusECSCluster --task `./aws ecs list-tasks --cluster $1-CumulusECSCluster --service-name $1-MDX --query "taskArns[0]" --region $AWS_REGION | tr -d '"'` --region $AWS_REGION
